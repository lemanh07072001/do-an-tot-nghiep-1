<x-app-layout>
    ...
    <div>
        <form method="POST" action="{{ route('ecommerce_module.voucher.update',$voucher) }}">
            @csrf
                ...
                <div class="col-span-8 ">
                    <x-card class="my-0">
                        ...
                        <div class="col-span-12 sm:col-span-3 ">
                            ...
                            <x-text-input name="name" :value="old('name')??$voucher->name" id="name" placeholder="Nhập mã" />
                        ...
                        <div class="grid grid-cols-12 ">
                            <div class="col-span-2 pr-2">
                                ...
                                <x-select name="discount_type">
                                    <option value="vnd" @selected(old('discount_type')??$voucher->discount_type == 'vnd')>Vnd</option>
                                    <option value="%" @selected(old('discount_type')??$voucher->discount_type == '%')>%</option>
                                </x-select>
                            </div>
                            <div class="col-span-10 pl-2">
                                <x-input-label for="value_reduction" required>Giá trị giảm</x-input-label>
                                <x-text-input name="value_reduction" class="int" :value="old('value_reduction')??$voucher->value_reduction"
                        ...
                        <div class="col-span-6 sm:col-span-3 ">
                            ...
                            <x-text-input class="{{ old('limit')??$voucher->limit != '' ? '' : 'cursor-not-allowed disabled' }}"
                                type="number" name="limit" :value="old('limit')??$voucher->limit" id="limit" placeholder="---" />

                            <label class="inline-flex items-center  cursor-pointer mt-2">
                                <input type="checkbox" class="sr-only peer unlimited" @checked(old('limit')??$voucher->limit != '')>
                                <div
                            ...
                            <x-select data-search="search-status" name="status">
                                @if (\App\Enums\Status::getValues())
                                    ...
                                    @foreach (\App\Enums\Status::getValues() as $status)
                                        <option value="{{ $status }}" @selected(old('status') && $voucher->status == $status)>
