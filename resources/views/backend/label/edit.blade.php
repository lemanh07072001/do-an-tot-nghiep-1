<x-app-layout>
    ...
    <div class="p-4 max-h-full block sm:flex items-center justify-between   dark:bg-gray-800 ">
        <div class="w-full mb-1">
            <div class="mb-4">
                {{ Breadcrumbs::render('labelUpdate', $label) }}
    ...
    <x-card>
        <form method="POST" action="{{ route('ecommerce_module.label.update', $label) }}">
            @csrf
            <div class="grid grid-cols-6 gap-4 gap-y-4 ">

                <div class="col-span-6 sm:col-span-3 ">
                    <x-input-label for="name" required>Tiêu đề</x-input-label>
                    <x-text-input name="name" :value="old('name')??$label->name" id="name" placeholder="Tiêu đề" />
                    <x-input-error error="name" class="mt-2" />
                </div>

                <div class="col-span-6 sm:col-span-3">
                    <x-input-label for="slug" required>Slug</x-input-label>
                    <x-text-input name="slug" :value="old('slug')??$label->slug" id="slug" />
                    ...
                    <x-select data-search="search-status" name="status">
                        @if (\App\Enums\Status::getValues())
                            ...
                            @foreach (\App\Enums\Status::getValues() as $status)
                                <option value="{{ $status }}" @selected(old('status', $label->status) == $status)>
    ...
    @push('js')
        ...
        <script>
            var imageUrl = @json(env('APP_URL') . $label->image);
