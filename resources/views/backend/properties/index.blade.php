<x-app-layout>
    @section('title', config('apps.properties.titleIndex'))
        ...
        <div class="w-full mb-1">
            ...
            <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
                ...
                <div class="flex items-center" id="handleAdd">
                    ...
                    <div class="flex pl-2 space-x-1 mr-2">
                        <x-dropdown-form>
                            ...
                            <x-slot:subDropdown class="w-60">
                                ...
                                <div class="p-1">
                                    <x-input-label required>Danh mục cha</x-input-label>
                                    <x-select data-search="search-parent">
                                        @if (\App\Enums\Status::getValues())
                                            <option value="">--Chọn danh mục--</option>
                                            @foreach ($getPropertiesSelectByParent as $item)
                    ...
                    <div>
                        @can('Thêm mới thuộc tính')
                            <a href={{ route('ecommerce_module.properties.create') }}
    ...
    @push('js')
        <script src="{{ asset('assets/apps/properties/datatables.js') }}"></script>

        <script>
            var url = '{!! route('ecommerce_module.properties.getData') !!}'
            const columns = [{
                    data: 'id'
                },
                {
            ...
            //NOTE - Toggle Status
            var urlToggleStatus = '{!! route('ecommerce_module.properties.toggleStatus') !!}'
            toggleStatus(urlToggleStatus, dataTableIndex)

            //NOTE - Delete All
            var urlToggleStatus = '{!! route('ecommerce_module.properties.deleteAll') !!}'
            deleteAll(urlToggleStatus, dataTableIndex)

            //NOTE - Delete Row
            var urlToggleStatus = '{!! route('ecommerce_module.properties.deleteRow') !!}'
