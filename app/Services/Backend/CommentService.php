    ...
    public function getData($request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('name', function ($comment) {
                ...
                return FormatFunction::formatTitleComment($comment);
            })
            ->addColumn('content', function ($comment) {
                return $comment->content;
            })
            ->addColumn('status', function ($comment) {
                return FormatFunction::formatStatusComment($comment);
            })
            ->addColumn('timedate', function ($comment) {
                return FormatFunction::formatDate($comment->created_at);
            })
            ...
            ->addColumn('action', function ($comment) {
                return '
                        <a href="' . route('ecommerce_module.comment.edit', ['comment' => $comment]) . '" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-yellow-400 hover:bg-yellow-800 focus:ring-4 focus:ring-yellow-300 dark:bg-yellow-400 dark:hover:bg-yellow-700 dark:focus:ring-yellow-800">
            ...
            ->rawColumns(['name',  'status', 'content',  'action'])
            ->make(true);
    }
